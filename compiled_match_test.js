...
for (var i = 0; i < tests.length; i++) {
	...
	var compiledPattern = CompiledMatch.compilePattern(test.p);
	var compiledMessage = CompiledMatch.compileMessage(test.m);
	...
	try {
	    ...
	    if (compiledPattern && compiledMessage) {
			for (var b = 0; b < rounds; b++) {
				CompiledMatch.compiledMatch(null, compiledPattern, compiledMessage, test.b);
			}
		} else {
			for (var b = 0; b < rounds; b++) {
	...
	var bss = null;
	if (compiledPattern && compiledMessage) {
		bss = CompiledMatch.compiledMatch(null, compiledPattern, compiledMessage, test.b);
	} else {
		bss = match(null, test.p, test.m, test.b);
	}
	result.bss = bss;
